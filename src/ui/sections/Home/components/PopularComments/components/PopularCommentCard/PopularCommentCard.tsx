...
export const PopularCommentCard = (props) => 
  // console.log(props.comment.release, props.comment.release.id ? props.comment.release.id : props.array.find(comment => props.comment.release === comment.release["@id"]).release.id);
   ...
   (
    <Link
      to={`/release/${props.comment.release.id ? props.comment.release.id : props.array.find((comment) => props.comment.release === comment.release['@id']).release.id}`}
      ...
      <div className={styles.comment}>
        <div className={styles.avatar}>
          <div className={styles.image_border}>
            <img
              src={props.comment.profile.avatar}
              className={styles.ava_image}
              alt={props.comment.profile.login + 'ava'}
            />
          </div>
        </div>
          ...
          <div className={styles.text_block}>
            <div className={styles.login_and_time}>
              ...
              <p className={styles.timestamp}>
                  {props.comment.release.title_ru
                    ? props.comment.release.title_ru
                    : props.array.find(
                        (comment) => props.comment.release === comment.release['@id']
                      ).release.title_ru}
                </p>
          ...
          <div className={styles.action_buttons}>
            <div className={styles.reply_and_like_buttons}>
              <div className={styles.dislike_and_like_button}>
                ...
                >
                  {unixToDate(props.comment.timestamp, 'full').slice(0, 7)} в{' '}
                  {unixToDate(props.comment.timestamp, 'full').slice(14)}
                </time>
