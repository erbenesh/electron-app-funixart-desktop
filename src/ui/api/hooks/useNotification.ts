...
export function useGetAllNotifications(token: string | null) {
    return useInfiniteQuery<PageableResponse<any>>({
        ...
        queryFn: ({ pageParam = 0 }) => notificationService.getAllNotifications(pageParam as number, token!),
        initialPageParam: 0,
...
export function useGetArticleCommentsNotifications(token: string | null) {
    return useInfiniteQuery<PageableResponse<any>>({
        ...
        queryFn: ({ pageParam = 0 }) => notificationService.getArticleComments(pageParam as number, token!),
        initialPageParam: 0,
        getNextPageParam: (lastPage) => {
            if (lastPage.currentPage < lastPage.totalPageCount - 1) {
                return lastPage.currentPage + 1;
...
export function useGetEpisodesNotifications(token: string | null) {
    return useInfiniteQuery<PageableResponse<any>>({
        ...
        queryFn: ({ pageParam = 0 }) => notificationService.getEpisodes(pageParam as number, token!),
        initialPageParam: 0,
        getNextPageParam: (lastPage) => {
            if (lastPage.currentPage < lastPage.totalPageCount - 1) {
                return lastPage.currentPage + 1;
...
export function useGetFriendsNotifications(token: string | null) {
    return useInfiniteQuery<PageableResponse<any>>({
        ...
        queryFn: ({ pageParam = 0 }) => notificationService.getFriends(pageParam as number, token!),
        initialPageParam: 0,
        getNextPageParam: (lastPage) => {
            if (lastPage.currentPage < lastPage.totalPageCount - 1) {
                return lastPage.currentPage + 1;
